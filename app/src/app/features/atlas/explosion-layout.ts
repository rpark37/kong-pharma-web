// Ported from ashemag/human-atlas app/explosion-layout.ts (MIT).
...
export function createExplosionLayout(parts:Part[],aspect=1){
 const cards=parts.map(p=>({id:p.id,system:p.system,width:Math.max(.035,p.sx)+.04,height:Math.max(.035,p.sy)+.04}));
