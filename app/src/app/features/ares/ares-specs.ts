...
const bar = (title: string, dataset: string, x: string, y: string, opts: Record<string, unknown> = {}): Record<string, unknown> => ({
  $schema: VL,
  title,
  width: 'container',
  height: 220,
...
export const REPORTS: Report[] = [
  {
    ...
    spec: {
      $schema: VL, title: 'Records by domain', width: 'container', height: 200, data: { name: 'recordsByDomain' },
  ...
  },
  {
    ...
    spec: {
      $schema: VL, title: 'Age at first observation', width: 'container', height: 220, data: { name: 'ageAtFirstObservation' },
  ...
  },
  {
    ...
    spec: { $schema: VL, title: 'Year of birth', width: 'container', height: 220, data: { name: 'yearOfBirth' }, mark: { type: 'area', line: true, interpolate: 'monotone', opacity: 0.35 }, encoding: { x: { field: 'year', type: 'quantitative', title: 'Year', axis: { format: 'd' } }, y: { field: 'persons', type: 'quantitative', title: 'Persons' }, tooltip: [{ field: 'year' }, { field: 'persons' }] } },
  },
  {
    id: 'sex', group: 'Person', title: 'Sex', description: 'Persons by administrative sex.', dataset: 'sex',
    spec: { $schema: VL, title: 'Sex', width: 'container', height: 200, data: { name: 'sex' }, mark: { type: 'arc', innerRadius: 50 }, encoding: { theta: { field: 'persons', type: 'quantitative' }, color: { field: 'sex', type: 'nominal', title: null }, tooltip: [{ field: 'sex' }, { field: 'persons' }] } },
  },
  {
    id: 'race', group: 'Person', title: 'Race', description: 'Persons by recorded race concept.', dataset: 'race',
    spec: { $schema: VL, title: 'Race', width: 'container', height: 200, data: { name: 'race' }, mark: { type: 'bar', cornerRadiusEnd: 2 }, encoding: { y: { field: 'race', type: 'nominal', sort: '-x', title: null }, x: { field: 'persons', type: 'quantitative', title: 'Persons' }, tooltip: [{ field: 'race' }, { field: 'persons' }] } },
  },
  {
    ...
    spec: bar('Length of observation (years)', 'observationLength', 'years', 'persons', { xTitle: 'Years observed', yTitle: 'Persons' }),
  },
  {
  ...
  },
  {
    ...
    spec: {
      $schema: VL, title: 'Age by sex', width: 'container', height: 240, data: { name: 'ageBySex' },
